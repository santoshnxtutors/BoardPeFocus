...
export function SchoolsCtaBlock({
  title,
  description,
  compact = false,
}: {
    ...
    <section className="rounded-[2.5rem] border border-border/60 bg-primary px-6 py-10 text-white shadow-[0_30px_80px_rgba(21,48,96,0.16)] md:px-10 md:py-12">
      <div
        className={
          compact
            ? "flex flex-col gap-8"
            : "grid gap-8 xl:grid-cols-[minmax(0,1.1fr)_minmax(20rem,0.9fr)] xl:items-center"
        }
      >
        <div>
        ...
        <div className={compact ? "grid gap-3 sm:grid-cols-2" : "grid gap-3 sm:grid-cols-2 2xl:grid-cols-3"}>
          <ButtonLink
            href="https://wa.me/918796367754?text=Hi%20BoardPeFocus%2C%20I%20need%20help%20with%20school-aware%20board%20tutoring%20in%20Gurgaon."
            target="_blank"
          ...
          <ButtonLink
            href="/search"
            variant="outline"
            className={`h-12 w-full justify-center rounded-xl border-white/20 bg-white/10 px-4 text-white hover:bg-white/15 hover:text-white ${compact ? "sm:col-span-2" : "sm:col-span-2 2xl:col-span-1"}`}
          >
            Get Matched
          </ButtonLink>
        </div>
