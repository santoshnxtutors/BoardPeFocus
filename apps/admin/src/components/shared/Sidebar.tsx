      ...
      <div className="p-6 bg-black/10">
        <div className="flex flex-col">
          <Link href="/dashboard" className="flex items-baseline leading-none tracking-tight">
            ...
            <span className="text-white text-2xl font-black mx-0.5">
              पे
            </span>
          ...
          <p className="text-[10px] font-black text-white/30 mt-1 uppercase tracking-widest">Super Admin Panel</p>
        </div>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto p-4 space-y-0.5 custom-scrollbar">
        {menuItems.map((item, i) => {
          if (item.type === "divider") {
            return (
              <p key={i} className="text-[9px] font-black text-slate-600 uppercase tracking-widest mt-6 mb-2 px-4">
                {item.label}
              </p>
            );
          }
